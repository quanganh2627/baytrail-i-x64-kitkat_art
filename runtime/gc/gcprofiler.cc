...
// Dump Record Header due to record type.
void RecordList::DumpHeader(std::ofstream& os) {
  switch (record_type_) {
    ...
    default: {
      os << "Not Supported Info!!" << std::endl;
      break;
    }
  }
}
...
void SuccAllocRecord::ConvertDataUnit() {
  if (GcProfiler::InaccurateMode()) {
    // Convert bytes to Mb.
...
// The size_dist_[] devided the object size into 11 regions.
...
// Convert the data unit for inaccurate mode.
void FailAllocRecord::ConvertDataUnit() {
  if (GcProfiler::InaccurateMode()) {
    // Convert bytes to Mb.
...
// Convert the data unit for inaccurate mode.
void LargeObjAllocRecord::ConvertDataUnit() {
  if (GcProfiler::InaccurateMode()) {
    // Convert bytes to Mb.
    ConvertSizeToMb(size_);
  }
...
void AllocInfoRecord::CalculateAllocThroughput(uint64_t duration) {
  if (duration != 0) {
    throughput_bpns_ = static_cast<double>(number_bytes_alloc_atomic_.LoadSequentiallyConsistent()) / static_cast<double>(duration);
    throughput_npns_ = static_cast<double>(number_objects_alloc_.LoadSequentiallyConsistent()) / static_cast<double>(duration);
  } else {
...
// Convert the data unit for inaccurate mode.
void AllocInfoRecord::ConvertDataUnit() {
  if (GcProfiler::InaccurateMode()) {
    // Convert bytes to Mb.
    number_bytes_alloc_ = number_bytes_alloc_atomic_.LoadSequentiallyConsistent() / (1024 * 1024);
    // Convert throughput.
