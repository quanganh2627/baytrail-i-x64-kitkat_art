...
class ProfileRecord {
 public:
  virtual void DumpRecord(std::ofstream& os) { }
  virtual ~ProfileRecord() { }
  // Coonvert time from ns to ms.
...
class GCRecord : public ProfileRecord {
 public:
  ...
  void UpdateWaitAndBlockTime(uint64_t wait_time,
    ...
    if (update_wait_time) {
      max_wait_time_ = wait_time > max_wait_time_ ? wait_time : max_wait_time_;
    }
    if (update_block_time) {
      blocking_time_ = wait_time > blocking_time_ ? wait_time : blocking_time_;
    }
  }
...
class GcProfiler {
 public:
  ...
  void InsertSuccAllocRecord(uint32_t byte_count, mirror::Class* klass) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);

  void EnableSuccAllocProfile(bool enable) {
    prof_succ_allocation_ = enable;
  }

  bool ProfileSuccAllocInfo() {
